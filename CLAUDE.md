...
- QF User API wire shape drifts from its published OpenAPI. Observed 2026-04-19: `/goals/get-todays-plan` requires `mushafId` query param despite the spec listing only `type` as required (returns 422); bookmark list rows omit `type` despite the spec marking it required + `additionalProperties:false`. Default stance: on Zod schemas in `src/lib/qf-schemas.ts`, keep only `id` and fields the adapter genuinely can't derive as required; everything else `.optional()` (and `.nullable()` where the spec allows). When a runtime field drops, update the adapter to infer from siblings (e.g. `toBookmark` uses `verseNumber` presence to distinguish ayah from surah bookmarks when `type` is absent) rather than re-tightening the schema.
- Demo-critical flows that fall back to sample data on error should surface at least one line of `QfApiError.status + message` diagnostic text alongside the user-friendly banner during investigation. The in-UI diagnostic pattern used on Day 8 (goals-context + bookmarks/page.tsx with a `font-mono text-[11px]` amber line showing the Zod issue path from `qfFetch`) cuts the round-trip to identify contract drift from three deploys to one. Revert the verbose text once the real fix is in.
