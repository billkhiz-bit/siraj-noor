...
- Scopes requested at sign-in: `openid offline_access bookmark collection reading_session goal streak` - deliberately scoped to only what the UI actually exercises. The `user` scope (full profile) and `post` scope (an unshipped social-sharing feature) are intentionally NOT requested: profile data is read straight from the id_token claims, and requesting scopes the app doesn't use is flagged by Quran Foundation's eligibility review.
